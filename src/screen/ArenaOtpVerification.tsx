...
export default function ArenaOtpVerificationScreen({ route, navigation }: any) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.9:8092/api/arena-owners/verify-otp', {
        method: 'POST',
        headers: {
  ...
  const handleSuccess = async () => {
    ...
    // Fetch owner details
    const userResponse = await fetch(`http://192.168.1.9:8092/api/arena-owners/by-phone?phoneNumber=${phoneNumber}`);
  ...
  const handleResend = async () => {
    try {
      const response = await fetch('http://192.168.1.9:8092/api/arena-owners/send-otp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
